...
def read_per_page(offset, limit, order_by, order, search_params, per_page, page, indirect_search_params):
    ...
    order_by_values = (
        'id', 'tipo_observacion_id', 'observacion', 'num_observacion'
    )
    ...
    table = 'observaciones_ires_cytg'
    join_details = {
        'direccion_id'      : ('observaciones_pre_cytg',     'id',           table,                    'observacion_pre_id', '',             False),
        'auditoria_id'      : ('observaciones_pre_cytg',     'id',           table,                    'observacion_pre_id', '',             False),
        'programa_social_id': ('observaciones_pre_cytg',     'id',           table,                    'observacion_pre_id', '',             False),
        'dependencia_id'    : ('auditoria_dependencias',     'auditoria_id', 'observaciones_pre_cytg', 'auditoria_id',       'auditoria_id', False),
        'anio_cuenta_pub'   : ('auditoria_anios_cuenta_pub', 'auditoria_id', 'observaciones_pre_cytg', 'auditoria_id',       'auditoria_id', False),
    }
    selects = ', observaciones_pre_cytg.direccion_id, observaciones_pre_cytg.auditoria_id, observaciones_pre_cytg.programa_social_id'
    joins, conditions, join_list = get_joins_and_conditions(indirect_search_params, join_details)
    join = ' JOIN observaciones_pre_cytg ON observaciones_pre_cytg.id = {}.observacion_pre_id'.format(table)
...
def seguimientos_to_comp_type_arr_lit(seguimientos):
    ...
    for s in seguimientos:
        ...
        segs_str += (
            ...
            "'" + s['num_oficio_ires'].replace("'", "''") + "', " +
            "'" + s['fecha_notif_ires'] + "', " +
            "'" + s['fecha_vencimiento_ires'] + "', " +
            str(s['prorroga']) + ", " +
            "'" + s['num_oficio_solic_prorroga'].replace("'", "''") + "', " +
            "'" + s['fecha_oficio_solic_prorroga'] + "', " +
            "'" + s['num_oficio_contest_prorroga'].replace("'", "''") + "', " +
